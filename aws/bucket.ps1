...
aws s3 mb "s3://$env:BUCKET_NAME" --region $env:AWS_REGION 2>$null
...
aws s3 cp ../lambda-feeder/target/lambda-feeder-1.0.0.jar "s3://$env:BUCKET_NAME/scripts/lambda-feeder.jar" | Out-Null
Write-Host "  lambda-feeder.jar uploaded" -ForegroundColor Gray

aws s3 cp ../lambda-ingest/target/lambda-ingest-1.0.0.jar "s3://$env:BUCKET_NAME/scripts/lambda-ingest.jar" | Out-Null
Write-Host "  lambda-ingest.jar uploaded" -ForegroundColor Gray

aws s3 cp ../lambda-mounter/target/lambda-mounter-1.0.0.jar "s3://$env:BUCKET_NAME/scripts/lambda-mounter.jar" | Out-Null
Write-Host "  lambda-mounter.jar uploaded" -ForegroundColor Gray

aws s3 cp ../lambda-query/target/lambda-query-1.0.0.jar "s3://$env:BUCKET_NAME/scripts/lambda-query.jar" | Out-Null
