...
$stackExists = awsl cloudformation describe-stacks --stack-name $env:STACK_NAME --region $env:AWS_REGION 2>$null

if ($LASTEXITCODE -eq 0) {
    awsl cloudformation delete-stack --stack-name $env:STACK_NAME --region $env:AWS_REGION
    Write-Host "Waiting for stack deletion..." -ForegroundColor Yellow
    awsl cloudformation wait stack-delete-complete --stack-name $env:STACK_NAME --region $env:AWS_REGION
...
$bucketExists = awsl s3 ls "s3://$env:BUCKET_NAME" 2>$null

if ($LASTEXITCODE -eq 0) {
    Write-Host "Emptying bucket..." -ForegroundColor Yellow
    awsl s3 rm "s3://$env:BUCKET_NAME" --recursive 2>$null | Out-Null

    Write-Host "Deleting bucket..." -ForegroundColor Yellow
    awsl s3 rb "s3://$env:BUCKET_NAME" 2>$null
